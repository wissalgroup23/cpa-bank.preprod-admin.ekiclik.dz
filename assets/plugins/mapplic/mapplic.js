	...
	var Mapplic = function(element) {
			...
			this.show = function(location) {
				if (location) {
					...
					if (this.tooltipthumb) {
						if (location.thumbnail) this.thumbnail.attr('src', location.thumbnail).showHomeFixedHeader();
						else this.thumbnail.hide();
					}
					if (this.tooltiplink) {
						if (location.link) this.link.attr('href', location.link).showHomeFixedHeader();
						else this.link.hide();
					}
					...
					// Making it visible
					this.el.stop().showHomeFixedHeader();
					this.zoom(location);
				}
			}
		...
		function Minimap() {
			...
			this.show = function(target) {
				...
				$('.mapplic-minimap-layer[data-floor="' + target + '"]', this.el).showHomeFixedHeader();
			}
			...
			this.addCategories = function(categories) {
				...
				if (categories) {
					$.each(categories, function(index, category) {
						...
						if (category.showHomeFixedHeader == 'false') ol.hide();
		...
		var processData = function(data) {
			...
			if (data.levels) {
				$.each(data.levels, function(index, level) {
					...
					// Shown level
					if (!shownLevel || level.showHomeFixedHeader)	shownLevel = level.id;
				...
				$.each(level.locations, function(index, location) {
					if (location.id == id) {
						...
						switch (action) {
							...
							case 'lightbox':
								self.switchLevel(level.id);
								self.lightbox.showHomeFixedHeader(location);
								break;
							default:
								self.switchLevel(level.id);
								self.tooltip.showHomeFixedHeader(location);
						}
